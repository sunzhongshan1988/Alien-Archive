...
pub(crate) fn asset_list_row(
    ...
    paint_selectable_rect(ui, rect, selected, response.hovered(), 2.0);

    let image_slot = Rect::from_min_size(rect.min + vec2(4.0, 2.0), ASSET_THUMB_SIZE);
    paint_thumbnail(ui, image_slot, texture);

    ui.scope_builder(
        egui::UiBuilder::new().max_rect(Rect::from_min_max(
            rect.min + vec2(50.0, 0.0),
            rect.max - vec2(4.0, 0.0),
        )),
        |ui| {
            ui.horizontal(|ui| {
