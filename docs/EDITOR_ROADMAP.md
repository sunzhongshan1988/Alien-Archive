...
- 资源面板：扫描 overworld PNG、导入未登记资产、编辑资产草稿、保存资产数据库、显示缩略图、资产依赖报告。
...
已完成基础报告：素材菜单里的“资产依赖报告”会汇总当前地图的缺文件、未登记 PNG、地图引用未知 asset、地图未使用 asset。
...
- P1 批量操作继续补齐：多选支持水平/垂直均匀分布，按对象显示边界在当前选区跨度内计算等间距；多选 Inspector 和编辑菜单新增“用当前素材替换所选”，会按图层类型匹配并跳过锁定/不兼容对象。
